...
class Dashboard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    account.when(
      data: (account) {
        sum = account.fold(0, (sum, account) => sum + account.balance);
      },
      loading: () => Center(child: CircularProgressIndicator()),
      error: (error, stack) =>
          Center(child: Text('Error loading accounts sum')),
    );
    final size = MediaQuery.of(context).size;
    final isLargeScreen = size.width >= 600; // tablet / laptop
    final isWideScreen =
        size.width / size.height > 1.2; // landscape or laptop-ish

    final double cardHeight = (isLargeScreen || isWideScreen)
        ? size.height * 0.50
        : size.height * 0.35;

    final double gridAspectRatio = (isLargeScreen || isWideScreen) ? 8 : 2.6;
    return Scaffold(
      appBar: AppBar(
        title: Text(
                ...
                children: [
                  ...
                  Expanded(
                    child: account.when(
                      data: (accountList) => GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          if (index < accountList.length) {
                            ...
                            return InkWell(
                              onTap: () =>
                                  openDialogueToAddAccount(context, ref),
                              child: Container(
                      ...
                      loading: () => Center(child: CircularProgressIndicator()),
                      error: (error, stack) =>
                          Center(child: Text('Error loading accounts')),
                    ),
                  )
                ],
              ),
